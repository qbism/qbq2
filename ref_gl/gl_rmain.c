...
static	qboolean gl_fogenabled = false;
static	float	gl_fogdensity = 50.0;
vec3_t gl_fogColor;
...
void R_SetFogVars(int density, int red, int green, int blue)
{
	...
	gl_fogenabled = (density > 0) ? true : false;
	if (!gl_fogenabled) { // recieved fog disable message
		qglDisable(GL_FOG);
		return;
	}
	gl_fogdensity = (float)density;
	
	gl_fogColor[0] = ((float)red) / 255.0;
	gl_fogColor[1] = ((float)green) / 255.0;
	gl_fogColor[2] = ((float)blue) / 255.0;

	// clamp vars
	gl_fogdensity = max(gl_fogdensity, 0.0);
	gl_fogdensity = min(gl_fogdensity, 100.0);
}
...
void R_RenderView (refdef_t *fd)
{
	...
	if (gl_fogenabled)
	{
		...
		qglFogf(GL_FOG_DENSITY, gl_fogdensity /20000.0);
...
qboolean R_SetMode (void)
{
	...
	if ( ( err = GLimp_SetMode( &vid.width, &vid.height, gl_mode->value) ) == rserr_ok )
	{
	...
	else
	{
		...
		if ( ( err = GLimp_SetMode( &vid.width, &vid.height, gl_state.prev_mode ) ) != rserr_ok )
		{
